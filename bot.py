...
# Command to organize nicknames based on a numbering convention
@bot.command(name="organize-nicknames", description="Organize nicknames based on a numbering convention.")
...
@bot.command(name="list-commands", description="Show available commands.")
async def list_commands(ctx: commands.Context):
    command_list = "\n".join([f"/{command.name}: {command.description}" for command in bot.commands if isinstance(command, commands.Command)])
